...
# Packages 	xfsprogs
# Inputs	EBS_DEVICE, EBS_MOUNT, USE_LOCAL_STORAGE
...
if [ $? == 0 ]; then
 ...
 echo "Make sure EBS_DEVICE is set correctly and the EBS volume is attached on boot"
...
if [ "$USE_LOCAL_STORAGE" = "NO" ]; then
  attach_device $EBS_DEVICE $EBS_MOUNT
fi
